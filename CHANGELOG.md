...
- Highlighting or commenting on text that spans more than one line now covers only the lines you selected. The blank page between two lines, and the space beside a short last line, stay clear — in the window, in a printed or flattened page, and in another PDF reader, where the annotation now carries one quadrilateral per line instead of one box over all of them. A comment made from a selection keeps its pin and its popup, and is exported as markup over that text carrying your note; a comment you drop on the page is still a note pinned to a point. Highlights and comments you place by hand are unchanged, and a document saved by an earlier version opens with its annotations exactly where they were.
