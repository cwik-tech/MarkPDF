...
- The fit menu now works on Markdown documents too. Fit to width scales the sheet so it fills the pane, and Actual size returns to 100%; the menu no longer offers Fit to page or Fit height for Markdown, because a continuous scrolling document has no page to fit. Choosing a zoom by hand settles the document back to Actual size, as it does for a PDF.
- The zoom controls now work on Markdown documents. Zooming in enlarges the sheet instead of doing nothing: the text grows and reflows across the wider sheet, and Mermaid charts, images and other SVGs scale up with it. The toolbar buttons and the `+`, `-` and `0` keyboard shortcuts behave the same as they do for a PDF, with the same 25%–400% range.
